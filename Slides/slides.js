...
const slide_id = pathname.includes("slide_1") ? "slide_1"
  : pathname.includes("slide_2") ? "slide_2"
  : "slide_3";

const slides = page_slides[slide_id];
const total_slides = slides.length;

function render_slide() {
  const slide = slides[current_slide];
  const content = document.querySelector(".content");
  const slide_show = document.querySelector(".slideshow");
  content.className = "content";
  content.innerHTML = "";
  slide_show.style.backgroundImage += `url(${slide.background.url})`;
  ...
  switch (slide.type) {
    ...
    case 2:
      ...
      content.innerHTML += `
        <h1 class="typewriter">${slide.title}</h1>
        <p class="fade-in">${slide.description[0]}</p>
      `;
    ...
    case 3:
      ...
      content.innerHTML += `
        <h1 class="fade-in">${slide.title}</h1>
        <p class="fade-in">${slide.description[0]}</p>
        <img class="fade-in" src="${slide.graphs?.[0]?.url}" alt="${slide.graphs?.[0]?.alt}" />
      `;
    ...
    case 4:
      ...
      content.innerHTML += `
        <h1 class="slide-in-left">${slide.title}</h1>

        <div class="slide-in-left">
          <p>${slide.description[0]}</p>
          <div>
            <img style="max-width: ${slide.raphs?.[0]?.maxWidth};" src="${slide.graphs?.[0]?.url}" alt="${slide.graphs?.[0]?.alt}" />
          </div>
        </div>

        <div class="second-div slide-in-right">
          <p>${slide.description[1]}</p>
          <div>
            <img src="${slide.graphs?.[1]?.url}" alt="${slide.graphs?.[1]?.alt}" />
          </div>
        </div>  
      `;
